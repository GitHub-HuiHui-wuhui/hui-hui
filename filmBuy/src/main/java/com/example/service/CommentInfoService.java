...
public class CommentInfoService {
    ...
    public PageInfo<CommentInfo> findPage(Integer pageNum, Integer pageSize, String name, HttpServletRequest request) {
        ...
        if (!CollectionUtil.isEmpty(all)) {
            for (CommentInfo commentInfo : all) {
                Integer level = commentInfo.getLevel();
                Long userId = commentInfo.getUserId();
                if (level == 1) {
                    commentInfo.setUserName(adminInfoService.findById(userId).getName());
                }
                if (level == 2) {
                    commentInfo.setUserName(userInfoService.findById(userId).getName());
                }
            }
        }
